...
import { AddSpecificNumbersFormProps, SingleNumberRow } from './AddSpecificNumbersForm.types';
import { styles } from './AddSpecificNumbersForm.styles';
...
const AddSpecificNumbersForm: React.FC<AddSpecificNumbersFormProps> = ({ countries }) => {
    ...
    return (
        <Paper variant="outlined" sx={styles.paper}>
            <Typography variant="subtitle1" sx={styles.subtitle1}>Add Specific Number(s)</Typography>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, marginBottom: 1 }}>
                ...
                <Typography sx={styles.rowCounter}>
                    {singleNumbers.length} row(s)
                </Typography>
            </Box>
            {singleNumbers.map(row => (
                <Box key={row.id} sx={styles.flexAlignCenterGap1Mb1}>
                    <Typography>Number</Typography>
                    <TextField
                        size="small"
                        sx={styles.width120}
                        value={row.number}
                        onChange={e => updateSingleRow(row.id, 'number', e.target.value)}
                    />
                    ...
                    <FormControlLabel
                        control={
                            <Checkbox
                                checked={row.serviceSms}
                                onChange={e => updateSingleRow(row.id, 'serviceSms', e.target.checked)}
                            />
                        }
                        label="SMS"
                    />
                    <FormControlLabel
                        control={
                            <Checkbox
                                checked={row.serviceVoice}
                                onChange={e => updateSingleRow(row.id, 'serviceVoice', e.target.checked)}
                            />
                        }
                        label="Voice"
                    />
            ...
            <Box sx={styles.flexGap1}>
                <Button
                    variant="contained"
                    size="small"
                    onClick={handleSaveSingleNumbers}
                    disabled={isAddingSingles || singleNumbers.length === 0}
                >
                    Save (add selected numbers)
                </Button>
            </Box>
        </Paper>
    );
