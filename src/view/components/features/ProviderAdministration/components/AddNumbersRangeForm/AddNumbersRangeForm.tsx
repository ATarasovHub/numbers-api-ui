...
import { AddNumbersRangeFormProps, NumberRange } from './AddNumbersRangeForm.types';
import { styles } from './AddNumbersRangeForm.styles';

const NEW_COUNTRY_VALUE = '__NEW__';

const AddNumbersRangeForm: React.FC<AddNumbersRangeFormProps> = ({
                                                                     countries,
                                                                     providerId
                                                                 }) => {
    ...
    const resolveRange = (): NumberRange | null => {
        const from = Number(fromNumber);
        const size = Number(rangeSize);
        if (fromNumber && rangeSize && !isNaN(from) && !isNaN(size) && size > 0) {
    ...
    return (
        <Paper variant="outlined" sx={styles.paper}>
            <Typography variant="subtitle1" sx={styles.subtitle1}>
                Add Number(s) <span style={{fontWeight:400, fontSize:'0.95em'}}>(range)</span>
            </Typography>
            <Box sx={styles.flexAlignCenterGap1Mb1}>
                <Typography>From Number</Typography>
                <TextField
                    size="small"
                    sx={styles.width120}
                    value={fromNumber}
                    onChange={e => { setFromNumber(e.target.value); setLastEdited('from'); }}
                />
                ...
                <TextField
                    ...
                    sx={styles.width120}
                    value={toNumber}
                    onChange={e => { setToNumber(e.target.value); setLastEdited('to'); }}
                />
                ...
                <TextField
                    ...
                    sx={styles.width60}
                    value={rangeSize}
                    onChange={e => { setRangeSize(e.target.value); setLastEdited('size'); }}
                />
            </Box>
            <Box sx={styles.flexAlignCenterGap1Mb1}>
                ...
                <FormControlLabel
                    control={<Checkbox checked={addServiceSms} onChange={e => setAddServiceSms(e.target.checked)} />}
                    label="SMS"
                />
                <FormControlLabel
                    control={<Checkbox checked={addServiceVoice} onChange={e => setAddServiceVoice(e.target.checked)} />}
                    label="Voice"
                />
            </Box>
            <Box sx={styles.flexGap1}>
                <Button
                    variant="contained"
                    size="small"
                    onClick={handleAddNumbers}
                    disabled={isAddingNumbers || !providerId}
                >
                    Save (add numbers)
                </Button>
            </Box>
